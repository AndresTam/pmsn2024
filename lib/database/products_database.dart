...
class ProductsDatabase {
    ...
    return openDatabase(
      ...
      onCreate: (db, version){
        String query = '''CREATE TABLE tblProductos(
          idProdcuto INTEGER PRIMARY KEY,
          nomProdcuto VARCHAR(30),
          canProducto INTEGER,
  ...
  Future<int> ACTUALIZAR(Map<String,dynamic> data) async{
    var conexion = await database;
    return conexion.update('tblProductos', data, where: 'idProducto = ?', whereArgs: [data['idProducto']]);
  }

  Future<int> ELIMINAR(int idProducto) async{
    var conexion = await database;
    return conexion.delete('tblProductos', where: 'idProducto = ?', whereArgs: [idProducto]);
  }
